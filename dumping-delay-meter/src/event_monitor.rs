use {
    ...
    web3::{
        transports::Http,
        types::{ Address, FilterBuilder, H256, U64, U256 },
        Web3,
        contract::Contract,
    },
...
const NEW_CONTRACT_EVENT_SIG: &str = "fcf9a0c9dedbfcd1a047374855fc36baaf605bd4f4837802a0cc938ba1b5f302";
...
async fn monitor_iteration(monitor: &EventMonitor, latest_block: U64, read_delay_spread_slots: u64) -> U64 {
    let new_latest_block = read_latest_block(&monitor.eth).await
        .expect("Unable to read latest block number");
    return if new_latest_block != latest_block {
        if let Ok(events) = read_events(
            ...
            for log in events {
                if let Some(log_block_number) = log.block_number {
                    let test_contract_addr = web3::types::Address::from(array_ref!(log.data.0, 12, 20));
                    info!("New Test contract {:?} created at block {:?}", test_contract_addr, log_block_number);
                    ...
                    tokio::spawn(async move {
                        ...
                        let abs_delay_slots = new_latest_block.as_u64() - log_block_number.as_u64() + rel_delay_slots;
                        let delay_ms = rel_delay_slots * 400;
                        info!("Delay read operation for {} ms", delay_ms);
                        tokio::time::sleep(std::time::Duration::from_millis(delay_ms)).await;
                        info!("Creating contract object...");
                        let test_contract: Contract<Http> = Contract::new(eth, test_contract_addr, test_contract_abi);
                        let log_block_number = web3::types::BlockId::Number(
                            web3::types::BlockNumber::Number(log_block_number)
                        );
                        let result: web3::contract::Result<U256> = test_contract.query(
                            "getCreationBlock", (), Address::default(),
                            web3::contract::Options::default(), log_block_number,
                        ).await;
    ...
    pub fn new(
        ...
        Self {
            ...
            topic: H256::from_str(NEW_CONTRACT_EVENT_SIG)
                .expect(format!("Failed to parse topic: {}", NEW_CONTRACT_EVENT_SIG).as_str()),
            pg_client,
        }
    }
